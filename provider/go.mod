...
// Pin grpc to a version patched against GHSA heap exhaustion via HTTP/2 DATA
// frame fragmentation and CVE-2026-84445 (xDS DoS via missing :authority/Host).
// grpc is only an indirect dependency here, so this replace keeps transitive
// requirements from pulling the module back below the patched release.
